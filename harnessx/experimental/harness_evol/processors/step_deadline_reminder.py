    ...
    async def on_step_start(self, event: StepStartEvent):
        ...
        if (
            ...
            early_reminder = (
                ...
                "`config.yaml` and a journal entry in `memo_path` "
        ...
        reminder = (
            ...
            "  2. Call `ValidateHarnessConfig` on your config and fix any errors\n"
            "  3. Journal entry appended to `memo_path` with `cited_candidates` frontmatter\n"
            "Write these now. Do not end your turn until all three are done.\n"
            f"{output_path_hint}"
        )
