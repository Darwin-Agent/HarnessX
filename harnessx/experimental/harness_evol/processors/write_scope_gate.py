...
def _extract_bash_write_targets(command: str) -> tuple[list[str], list[str]]:
    ...
    while i < len(tokens):
        ...
        # mkdir/touch/ln/install — all non-flag arguments are write targets.
        # mkdir -p /a /b  → both /a and /b are targets
        # ln -s src dest  → dest is a target
        # install -m 755 src dest  → dest is a target (last arg)
        # touch /a /b     → both are targets
        if tok in {"mkdir", "touch"}:
            j = i + 1
            while j < len(tokens):
                t = tokens[j]
                if t in _SHELL_CONTROL:
        ...
        if tok in {"ln", "install"}:
            ...
            while j < len(tokens):
                ...
                if not t.startswith("-"):
                    args.append(t)
                j += 1
            # ln: last arg is destination; install: last arg is destination
            if args:
                _record_target(args[-1])
            i = j
            continue
